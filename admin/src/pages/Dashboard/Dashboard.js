...
const Dashboard = () => {
  ...
  return (
    <div className="dashboard-page-wrapper">
      ...
      {/* Recent Orders Table inside a standard UI Card */}
      <div className="ui-card recent-orders-card">
        <div className="card-header flex-between">
          <h3>Recent Orders</h3>
          <a href="/orders" className="ui-link-btn">View All Orders</a>
        </div>
              ...
              <tbody>
                {stats.recentOrders.map(order => (
                  <tr key={order.id}>
                    ...
                    <td>
                      <span className={`ui-badge badge-${(order.status || 'pending').toLowerCase()}`}>
                        {order.status}
                      </span>
                    </td>
